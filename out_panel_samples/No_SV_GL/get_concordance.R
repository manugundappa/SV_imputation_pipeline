...
###loop for SNP concordance generation
for(i in ids)
{
  #just read truth set once
  Sys.setenv("VROOM_CONNECTION_SIZE" = 131072 * 400)
  filesT<-dir(pattern = glob2rx(paste("aqualeap_snps.",i,".*.vcf.gz", sep=""))) ### true SNP calls generated using 15x data
  ...
  filesI<-dir(pattern = glob2rx(paste(i, "*.",cov,".00.vcf", sep=""))) ##imputed output vcf files 
...
###loop for SV concordance generation
for(i in ids)
{
  #just read truth set once
  Sys.setenv("VROOM_CONNECTION_SIZE" = 131072 * 400)
