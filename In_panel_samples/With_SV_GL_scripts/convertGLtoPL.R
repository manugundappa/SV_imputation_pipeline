...
sv<-read_tsv(paste("genotype_",cov,"_chromosome_vcfs/",id,".",chr,".",cov,"-joint-smoove.genotyped.vcf.gz",sep=""), comment="##") %>% 
  separate(id, sep=":", into=c("GT", "GQ", "SQ", "GL", "DP", "RO", "AO", "QR", "QA", "RS", "AS", "ASC", "RP", "AP", "AB", "DHFC", "DHFFC", "DHBFC", "DHSP")) %>%
  filter(GQ > 0)
...
sv[,id]<-paste(sv$GT, ":", sv$PL1, ",", sv$PL2, ",", sv$PL3, ":", sv$DP, sep="")

sv$FORMAT<-"GT:PL:DP"

sv<-sv %>% select(c("#CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT", id))
